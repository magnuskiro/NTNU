...
public class ICDsearch {
     ...
     */
    public void index(Case currentCase, ArrayList<ICD> icds, int limit){
        ...
        try {
            ...
            for(Sentence s: currentCase.getSentences()){
                ...
                for (int i = 0; i < hits.length; i++) {
                    ...
                    ICD icd = getAtc(hitDoc.get("label"), icds);
                    s.addIcd(i, icd);
    ...
    public ICD getAtc(String label, ArrayList<ICD> icds){
        for(ICD icd: icds){
            if (icd.getLabel() == null) continue;
            if(icd.getLabel().equals(label)) return icd;
        }
        return null;
    }
