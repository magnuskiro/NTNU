...
public class ServerImpl extends UnicastRemoteObject implements Server
{
	...
	@Override
	public void receiveProbe(Queue<Integer> probe) throws RemoteException {
		if(this.activeTransaction != null){
			LockedResource waitingFor = this.activeTransaction.getWaitingForResource();
			if(waitingFor != null){
				if(probe.contains(this.getServerId())){
					//Possible deadlock
					//Very simple deadlock resolution, just kill the one who
					//noticed that we got one round in the graph
					probe.add(this.getServerId());
					this.println("Deadlock detected: " + probe, this.activeTransaction.getId());
					this.endCurrentTransaction();
				}else{
					this.println("Waiting for other transaction, sending probe message to it", 
							this.activeTransaction.getId());
					probe.add(this.getServerId());
					//Send message on to the transaction we are waiting for
					waitingFor.location.receiveProbe(probe);
				}
			}
		}
	}
	...
	public void notifyResource(int resourceID) throws RemoteException{
		Resource res = (Resource) this.resources.get(resourceID);
		synchronized (res) {
			res.notify();
		}
	}
	...
	private void endCurrentTransaction(){
		this.println("Trying to resolve deadlock", this.activeTransaction.getId());
		try {
			this.activeTransaction.getWaitingForResource().location.notifyResource(this.activeTransaction.getWaitingForResource().resourceId);
