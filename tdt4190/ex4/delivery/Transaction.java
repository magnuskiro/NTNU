...
public class Transaction
{
	...
	private ArrayList<LockedResource> lockedResources;
	...
	public boolean runTransaction() {
		...
		lockedResources = new ArrayList<LockedResource>();
	...
	public void acquireLock(LockedResource resource) {
		...
		owner.println("Trying to claim lock of resource " + resource.resourceId +
				" at server " + resource.locationId, transactionId);
		try {
			if(Globals.PROBING_ENABLED)
				new Prober(owner).start();
			if(resource.location.lockResource(transactionId, resource.resourceId)) {
				lockAcquired(resource);
				return;
			}else{
				//This means that the lock timed out
	...
	public synchronized void serverDisconnected(int serverId) {
		if(lockedResources != null) {
			for(int i = 0; i < lockedResources.size(); i++) {
				if(lockedResources.get(i).locationId == serverId) {
	...
	private synchronized void releaseLocks() {
		while(lockedResources.size() > 0) {
			LockedResource lr = lockedResources.remove(lockedResources.size()-1);
			releaseLock(lr);
		}
		if(input != null) {
